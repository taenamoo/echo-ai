...
* Frontend handling: the SPA API client (`apps/spa/src/api.ts`) clears invalid/expired tokens and redirects to `/auth/login?reason=expired`. It also emits a `window` event `auth:session-expired` so the UI can show a toast/modal if desired.
