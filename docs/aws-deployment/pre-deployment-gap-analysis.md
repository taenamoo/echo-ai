...
`docs/aws-deployment/current-deployment-steps.md` 문서의 체크리스트를 실행하기 전에, 현행 코드베이스와 IaC 정의에서 배포를 가로막거나 품질을 저하할 수 있는 미완료 항목을 정리했다. 아래 목록은 우선순위별로 나누어 정리했으며, 각 항목은 관련 소스와 추가 후속 조치를 함께 명시한다.
...
- ✅ **Next.js API 경로가 Lambda와 이중 구현으로 남아 품질 리스크**
  - *조치 완료*: App Router의 `/api/**` 라우트를 제거하고, Axios 기본 URL을 `NEXT_PUBLIC_API_BASE_URL`에 맞춰 API Gateway로 직접 호출하도록 클라이언트를 정비했다. 업로드·요약·스터디 화면 모두 Lambda 엔드포인트만 사용해 Next 런타임 비밀 의존을 제거했다. 【F:apps/web/src/lib/axios.ts†L1-L29】【F:apps/web/src/app/documents/page.tsx†L1-L620】【F:apps/web/src/app/study/page.tsx†L1-L560】
...
- ✅ **운영 문서와 실제 구현 정합성 확보**: 아키텍처 문서의 격차 목록을 최신 상태로 정비해 Secrets Manager 통합과 Next.js API 제거가 완료되었음을 반영했다. 앞으로 해결해야 할 항목만 남도록 `docs/architecture/current-system.md`를 업데이트했다. 【F:docs/architecture/current-system.md†L45-L73】
...
- 🟠 **남은 리스크 문서화 및 후속 계획**: Secrets 회전 자동화, 운영 모니터링, 레거시 앱 정리 등 잔여 이슈는 `docs/architecture/current-system.md`에 남겨두고 후속 계획을 수립 중이다. 【F:docs/architecture/current-system.md†L1-L90】

위 항목을 기반으로 `docs/aws-deployment/current-deployment-steps.md`의 배포 절차를 안전하게 진행할 수 있으며, 미완료 항목은 운영 준비 단계에서 계속 추적한다.
