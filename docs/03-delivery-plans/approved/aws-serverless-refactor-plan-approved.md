---
title: AWS 서버리스 아키텍처 적용을 위한 프로젝트 구조 개편 계획서
...
> 참고: `apps/web`로 Next.js 코드를 이동하고, `packages/@echo-ai/*` 워크스페이스로 공통 로직을 추출합니다. 기존 `scripts` 폴더는 성격에 따라 `scripts/local` 또는 `packages/core-domain` 테스트 코드로 재배치합니다.
...
| **2. 공통 패키지 추출** | 1.5주 | `packages/@echo-ai/*` 워크스페이스로 문서/인증/설정 로직을 분리하고 유닛 테스트 추가 | ✅ `@echo-ai/*` 패키지로 핵심 로직 분리, 후속 테스트 정비 예정 |
...
| **6. 문서/운영 정리** | 0.5주 | 운영 Runbook, 알람 전략, 비용 모니터링 문서화 | docs/04-operations/approved/runbooks 업데이트, CloudWatch 알람 작동 확인 |

> 각 단계 완료 시 `docs/03-delivery-plans/approved`에 작업 회고/체크리스트를 업데이트하고, develop 환경으로 부분 배포하여 회귀 테스트를 수행합니다.
