...
1. Presigned 업로드 URL 발급 API — docs/03-delivery-plans/approved/documents-presign-plan-approved.md
2. 문서 목록/상세/삭제 API — docs/03-delivery-plans/approved/documents-crud-plan-approved.md
3. 문서 요약 API — docs/03-delivery-plans/approved/documents-summarize-plan-approved.md
4. 문서 목록/상세/업로드/요약 UI — docs/03-delivery-plans/approved/documents-frontend-plan-approved.md
5. 인증/보안 강화 — docs/03-delivery-plans/approved/auth-hardening-plan-approved.md
6. 인증 UI (로그인/회원가입/로그아웃) — docs/03-delivery-plans/approved/auth-ui-plan-approved.md
7. 테스트 및 모니터링 — docs/03-delivery-plans/approved/testing-monitoring-plan-approved.md
...
- 정렬: `sortKey`(`createdAt|updatedAt|filename|filesize`), `sortDir`(`asc|desc`) 지원(응답 내 정렬)
 ...
 - 동적 라우트 마이그레이션: App Router 변경사항에 따라 `params`는 Promise로 전달되며, 페이지 컴포넌트에서 `use(params)`로 언랩하여 사용
