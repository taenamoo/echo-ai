---
title: 작업 계획서: 인증/보안 강화 및 라우트 정리
...
- FE에서 401(만료/무효) 수신 시 토큰 삭제 및 재로그인 유도 — 충족(`apps/spa/src/api.ts` 인터셉터)
...
- 프론트엔드
  - `apps/spa/src/api.ts` 공용 클라이언트에 401 처리 로직 추가 → 만료/무효 시 토큰 삭제+리다이렉트
