---
title: 현행 시스템 → 목표 아키텍처 전환 작업 계획서
domain: architecture
status: approved
...
## 1. 배경 및 목표
- **배경**: `docs/02-architecture/approved/current-system-overview-approved.md`에서 정의된 Next.js 모놀리식 구조는 동기식 Gemini 요약, .env 기반 비밀 관리 등 여러 제약을 갖고 있다.
- **목표**: `docs/02-architecture/approved/target-architecture-overview-approved.md`에 정의된 AWS 서버리스 기반 구조로 이전하여 자동화된 배포, 비동기 요약 처리, Secrets Manager 연동을 달성한다.
...
### 단계 1. 설계 정교화 및 준비 (완료)
- 결과물: [`docs/02-architecture/approved/stage-01-prep-decisions-approved.md`](./step-01-prep-decisions.md), [`docs/02-architecture/approved/iac-guidelines-approved.md`](./iac-guidelines.md)
...
- 산출물/수용 기준
  ...
  - 상세 계획서는 `docs/02-architecture/proposals/simplification-stage-04-plan-draft.md` 참조.
...
## 4. 산출물
- 업데이트된 시스템 문서: `docs/02-architecture/approved/target-architecture-overview-approved.md` 반영 + 전환 결정 사항.
