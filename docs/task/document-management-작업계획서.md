...
9) 인증/보안 정비
- 라우트 정리: `src/app/api/signup/route.ts` 제거(중복), `/api/auth/*` 네임스페이스로 일원화
- 토큰 만료: Access Token 만료(예: 1시간) 적용 및 검증 강화
- 비밀번호 정책: 최소 길이, 문자 조합 체크
- (선택) 이메일 검증·재설정: 추후 이슈 분리
- 수용 기준: 불필요 라우트 제거, 만료 토큰 거절, 기본 정책 충족
