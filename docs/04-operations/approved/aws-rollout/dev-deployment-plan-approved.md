---
title: AWS 개발 서버 배포 계획서 (Lambda-first + SPA)
domain: operations
status: approved
...
본 문서는 현행 코드베이스(`docs/02-architecture/approved/current-system-overview-approved.md`)와 목표 아키텍처(`docs/02-architecture/approved/target-architecture-overview-approved.md`)를 기준으로 개발 환경(dev) 배포를 위해 지금 수행해야 할 실무 작업을 정리한다. 하위 문서(`docs/04-operations/approved/aws-rollout/**`)의 단계별 가이드를 참조하여 순차 수행한다.
...
## 8) 검증·운영 준비
- 네트워크 연결 테스트: `docs/04-operations/approved/aws-rollout/step-62-network-connectivity-test.md` 절차 수행。
...
## 10) 레퍼런스(문서 매핑)
- 현행/목표: `docs/02-architecture/approved/current-system-overview-approved.md`, `docs/02-architecture/approved/target-architecture-overview-approved.md`
