...
## 7. 구조적 복잡성 요약
- 이중 어댑터: Next.js API Route와 Lambda 어댑터가 공존해 동일 기능의 구현·검증·에러 규약이 중복 관리된다.
- 동기/비동기 혼재: 문서 요약이 동기 경로와 SQS 기반 비동기 경로로 분기되어 타임아웃·재시도·SLA 정책이 분산된다.
- 서버 업로드 분기: 프리사인드 업로드와 서버사이드 업로드(멀티파트)가 혼재해 확장자/MIME/사이즈 정책 일관성이 떨어진다.
- 환경/비밀 이원화: 로컬은 `.env` 기반, 목표는 Secrets Manager 기반으로 정의되어 단계별 구성이 엇갈린다.
- 로컬/클라우드 파리티 부족: 일부 플로우는 Next.js 라우트를, 일부는 Lambda 로컬 러너를 사용하여 동작 편차가 발생한다.

## 8. 단순화 원칙(제안)
- Lambda First: 서버 기능의 단일 출처를 Lambda로 통일하고, 어댑터 중복을 제거한다.
- UI Static: UI는 정적 SPA로 전환(S3+CloudFront), 서버 런타임 의존 제거.
- Presign Only: 파일 업로드는 프리사인드 POST만 허용, 서버사이드 업로드 경로 폐기.
- Async Only: 요약은 기본 202 큐잉(SQS)으로 일원화, 동기 요약 경로 제거.
- 단일 계약: 요청/응답 스키마·에러 규약을 `@echo-ai/api-core`에서 zod 기반으로 통일.
- 로컬=클라우드: 로컬에서도 Lambda 핸들러를 HTTP로 에뮬레이션해 동일 엔드포인트/계약으로 개발.
