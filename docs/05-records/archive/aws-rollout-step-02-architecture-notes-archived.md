---
title: Step-02 배포 대상 애플리케이션 구조 파악 이행 기록
domain: records
status: archived
...
| --- | --- | --- | --- |
| 프런트엔드 + SSR(레거시) | Next.js 15(App Router), React 19, Tailwind CSS 4 | (현재는 제거됨, 과거에는 별도 Next.js 워크스페이스를 `pnpm --filter`로 실행) | `Dockerfile` 및 `docker-compose.yml`로 로컬 실행, Node 20 기반 |
...
| 아키텍처 다이어그램 부재 | 서버리스 목표 구조만 문서화 | `docs/02-architecture/approved/current-system-overview-approved.md/.drawio`에 현행/목표 비교 섹션 추가 | draw.io 또는 Excalidraw로 제작 가능하며, 이해관계자 검토 후 Step-03 저장소 전략 수립에 활용 |
...
- (작성 예정) 시스템 구성 요약: `docs/02-architecture/approved/current-system-overview-approved.md`.
