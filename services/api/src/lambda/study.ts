...
function respond(
  ...
  return jsonResponse(statusCode, body, event.headers ?? {});
}
...
export const quiz: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    ...
    const body = event.body ? safeJson<any>(event.body) : null;
    const content = body?.content;
    const count = Number(body?.count ?? 5);
    if (!content || typeof content !== 'string') {
      return respond(event, 400, { error: '퀴즈 내용을 찾을 수 없습니다.' });
    }
    if (!Number.isFinite(count) || count <= 0) {
      return respond(event, 400, { error: 'count 값이 올바르지 않습니다.' });
    }
