...
export default function StudyNotes() {
  ...
  async function addRoot() {
    ...
    setBusy(true);
    try {
      await createStudy({ title, study_order: 0 });
      await refresh();
    } finally {
      setBusy(false);
    }
  ...
  async function addChild(parentId: string) {
    ...
    setBusy(true);
    try {
      await createStudy({ title, study_order: 0, parent_id: parentId });
  ...
  function sortByOrder<T extends { study_order?: number }>(arr: T[]): T[] {
    return [...arr].sort((a, b) => (a.study_order || 0) - (b.study_order || 0));
  }
  function filterTree(nodes: Study[], keyword: string): Study[] {
    if (!keyword.trim())
      return sortByOrder(nodes).map((n) => ({
        ...n,
        children: sortByOrder(n.children || []),
      }));
    ...
    return sortByOrder(filtered).map((n) => ({
      ...n,
      children: sortByOrder(n.children || []),
    }));
  }
  ...
  async function remove(study: Study) {
    if (!window.confirm('삭제하시겠습니까? 하위 항목도 함께 삭제됩니다.'))
      return;
    setBusy(true);
    try {
    ...
    <div className="flex h-full">
      <aside className="w-1/4 min-w-[280px] bg-white p-6 overflow-y-auto border-r border-gray-200">
              ...
              >
                ...
                <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      void addChild(main.study_id);
                    }}
                    className="text-sky-500 hover:bg-sky-100 p-1 rounded-full"
                  >
                    +
                  </button>
                  <button
              ...
              {(main.children || []).length > 0 && (
                <div className="ml-4 mt-1 pl-2 border-l-2 border-gray-200 space-y-1">
                  {sortByOrder(main.children || []).map((sub) => (
                    <div
                      key={sub.study_id}
                      className={`group flex justify-between items-center p-2 rounded-md cursor-pointer transition-colors text-sm ${selected?.study_id === sub.study_id ? 'bg-sky-100 text-sky-700' : 'text-gray-600 hover:bg-gray-100'}`}
        ...
        <button
          ...
          disabled={busy}
          className="w-full mt-6 bg-sky-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-sky-600 transition-colors text-sm"
        >
          + 새 상위 메뉴
        </button>
      </aside>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-lg shadow-lg">
            <div className="flex justify-between items-center mb-6 pb-6 border-b">
              ...
              <div className="flex gap-2">
                {!selected.parent_id && (
                  <button
                    onClick={() => void addChild(selected.study_id)}
                ...
                {selected.parent_id && (
                  <button
                    ...
                    className="bg-sky-500 text-white font-semibold py-2 px-4 rounded-lg hover:bg-sky-600 transition-colors"
                  >
                    AI 분석
                  </button>
                )}
                <button
                  ...
                  className="bg-white border text-gray-700 font-semibold py-2 px-4 rounded-lg hover:bg-gray-50 transition-colors"
                >
                  수정
                </button>
                <button
              ...
              <div className="mb-8 border rounded-md p-4 bg-gray-50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <label className="block">
                    ...
                    <input
                      type="number"
                      value={form.study_order}
                      onChange={(e) =>
                        setForm({
                          ...form,
                          study_order: Number(e.target.value || 0),
                        })
                      }
                      className="mt-1 w-full p-2 border rounded"
                    />
                ...
                <div className="mt-4">
                  <label className="block w-full">
                    ...
                    <textarea
                      ...
                      className="mt-1 w-full p-2 border rounded min-h-[120px]"
                    />
                  </label>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  <label className="block">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setForm({ ...form, good_example: e.target.value })
                      }
                      className="mt-1 w-full p-2 border rounded min-h-[100px] font-mono"
                    />
                  </label>
                  <label className="block">
                    <span className="text-sm text-gray-700">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setForm({ ...form, bad_example: e.target.value })
                      }
                      className="mt-1 w-full p-2 border rounded min-h-[100px] font-mono"
                    />
                  </label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                <div>
                  ...
                  <pre className="bg-gray-800 text-white p-4 rounded-lg text-sm overflow-x-auto">
                    <code>{selected.good_example || ''}</code>
                  </pre>
                </div>
                <div>
                  ...
                  <pre className="bg-gray-800 text-white p-4 rounded-lg text-sm overflow-x-auto">
                    <code>{selected.bad_example || ''}</code>
                  </pre>
                </div>
              </div>
            )}
            {selected.ai_suggestion && (
              <div className="mt-8">
                <h3 className="font-bold text-lg mb-2 text-sky-600">
                  AI 추가의견
                </h3>
                <div
                  className="prose max-w-none text-gray-600 leading-relaxed bg-sky-50 p-4 rounded-md"
                  dangerouslySetInnerHTML={{
                    __html: selected.ai_suggestion.replace(/\n/g, '<br />'),
                  }}
                />
              </div>
            )}
